...
export default function ScenarioActivityPage() {
  ...
  const getScenarioImage = () => {
    ...
    switch (category) {
      case 'greeting':
        return SCENARIO_IMAGES.greeting;
      case 'sharing':
        return SCENARIO_IMAGES.sharing;
      case 'helping':
        return SCENARIO_IMAGES.helping;
      case 'takingTurns':
        return SCENARIO_IMAGES.takingTurns;
      default:
        return SCENARIO_IMAGES.greeting;
    }
  };

  const handleChoiceSelect = (index: number, choice: Choice) => {
        ...
        <div className="bg-white rounded-3xl shadow-2xl p-8">
          <div className="text-center mb-6">
            <h1 className="text-4xl font-bold text-purple-600 mb-4">
              {activity?.title ?? 'Social Scenario'}
            </h1>
          </div>
            ...
            {choices?.map?.((choice, index) => {
              ...
              return (
                <button
                  ...
                  className={`w-full child-button text-left px-8 ${
                    showCorrect ? 'bg-green-400 text-white' : ''
                  } ${
                    showWrong ? 'bg-red-400 text-white' : ''
                  } ${
                    !isSelected ? 'bg-gradient-to-r from-purple-400 to-pink-400 text-white' : ''
                  } disabled:opacity-70`}
                >
              ...
              <div
                className={`inline-block px-8 py-4 rounded-3xl ${
                  choices[selectedChoice]?.isCorrect
                    ? 'bg-green-100 border-4 border-green-400 text-green-700'
                    : 'bg-orange-100 border-4 border-orange-400 text-orange-700'
                }`}
              >
